...
if options == "Score Client":
    ...
    if X_test_original is not None and X_test_clean is not None:
        ...
        st_shap(shap.force_plot(explainer_shap.expected_value[1], shap_values[1][idx, :], X_test_clean.drop(labels="SK_ID_CURR", axis=1).iloc[idx, :], link='logit', figsize=(20, 8), ordering_keys=True, text_rotation=0, contribution_threshold=0.05))
        ...
        st_shap(shap.decision_plot(explainer_shap.expected_value[1], shap_values[1][idx, :], X_test_clean.drop(labels="SK_ID_CURR", axis=1).iloc[idx, :], feature_names=X_test_clean.drop(labels="SK_ID_CURR", axis=1).columns.to_list(), feature_order='importance', feature_display_range=slice(None, -16, -1), link='logit'))
